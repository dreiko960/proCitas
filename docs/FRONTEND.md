...
| `localStorage` + evento `storage` | API + **Socket.IO** (cola y TV en vivo entre dispositivos) |
...
4. **Tiempo real vía Socket.IO** solo donde aporta (cola, TV, notificaciones); el resto es REST.
...
| Tiempo real | **socket.io-client** | Cola + TV + notificaciones |
...
└── src/
    ...
    │   └── socket.js               # cliente Socket.IO (cola, TV, notificaciones)
    ├── auth/
...
export function useQueueDay(date) {
  ...
  const { data } = useQuery({
    ...
    refetchInterval: 15_000,          // respaldo si el socket falla
  })
  useEffect(() => {
    const s = socket.on('queue.updated', () => queryClient.invalidateQueries(['queue']))
    return () => s.off('queue.updated')
  }, [queryClient])
  return data
}
```
...
| Cola + TV | `WaitingQueue.jsx`, `TvDisplay.jsx` | Suscripción Socket.IO (Parte 8); el "Modo automático demo" se elimina (o queda en staging) |
...
- `TvDisplay.jsx` se conserva en visual; la fuente de datos cambia:
  1. `POST /tv/token` (clave de consultorio) → token `aud: 'tv'` en `sessionStorage`.
  2. `GET /queue/day?date=hoy` → render inicial.
  3. `socket.io-client` → namespace `/queue`, room `consultorio:{id}`; eventos `queue.updated` (recarga payload) y `turn.called` (animación del turno en grande).
  4. Reconexión con backoff exponencial + `GET /queue/day` al reconectar.
...
| `WaitingQueue.jsx` (estado en memoria) | `useQueue` + socket `queue.updated` |
| `TvDisplay.jsx` (modo automático) | Kiosco con token TV + socket (sin auto-demo) |
